...
def game():
    ...
    parser.add_argument(
        "--map",
        type=str,
        help='Name of the map file to load from the maps directory (e.g., "test.csv")',
    )
    parser.add_argument(
        "--generate",
        action="store_true",
        help="Generate a new random maze and use it immediately.",
    )
    ...
    if args.generate:
        ...
        if not available_maps:
            print(
                "Error: No map files (.csv) found in the maps directory.",
                file=sys.stderr,
            )
            return
